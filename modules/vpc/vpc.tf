...
resource "aws_vpc" "this" {
  ...
  assign_generated_ipv6_cidr_block = true // Automatically assigns an IPv6 CIDR block to the VPC
...
resource "aws_internet_gateway" "this" {
  ...
  tags = merge(
    {
      Name = "${var.project}-igw" # If there is the Name tag is defined in var.tags, var.tags will be overridden
    },
    var.tags # So try not to include Name tag in var.tags
  )
}
...
resource "aws_subnet" "public_subnet" {
  ...
  ipv6_cidr_block         = cidrsubnet(aws_vpc.this.ipv6_cidr_block, 8, count.index) // Allocates IPv6 CIDR block to each public subnet
  map_public_ip_on_launch = true                                                     // Automatically assigns public IP to instances in these subnets.
  availability_zone       = element(var.availability_zones_public, count.index)      // Sets the availability zone for each subnet.

  tags = merge(
    {
      Name = "${var.project}-public-subnet-${element(var.availability_zones_public, count.index)}"
...
# 4.1 Defining a route in the public route table to the Internet Gateway (IPv4)
resource "aws_route" "route_to_igw_ipv4" {
...
# 4.2 Defining a route in the public route table to the Internet Gateway (IPv6)
resource "aws_route" "route_to_igw_ipv6" {
  route_table_id              = aws_route_table.public_route_table.id // The ID of the public route table.
  destination_ipv6_cidr_block = "::/0"                                // Represents all IPv6 addresses (internet traffic).
  gateway_id                  = aws_internet_gateway.this.id          // The ID of the Internet Gateway
}

# 4.3 Route table associations (if there are 3 subnets, 3 subnets will be associated with that public route table )
...
resource "aws_nat_gateway" "this" {
  ...
  subnet_id  = element(aws_subnet.public_subnet.*.id, 1) // nat gateway will be created in public subnet (check subnet_id is of public subnet)
...
resource "aws_subnet" "private_subnet" {
  ...
  ipv6_cidr_block         = cidrsubnet(aws_vpc.this.ipv6_cidr_block, 8, length(var.public_subnets_cidr) + count.index) // Allocates IPv6 CIDR block to each private subnet
  availability_zone       = element(var.availability_zones_private, count.index)                                       // Sets the availability zone for each subnet.
  map_public_ip_on_launch = false                                                                                      # Disables public IP assignment on instance launch in this subnet ( as it is private subnet )

  tags = merge(
    {
      Name = "${var.project}-private-subnet-${element(var.availability_zones_private, count.index)}"
      #     "kubernetes.io/cluster/${var.project}-cluster" = "shared"
